...
/* Usage:
	...
	if(sendmail( 'to@example.com', 'subject', '<p>html message</p>' )){
		echo "Message sent!";
	}else echo "Mailer Error: " . $sendmail_error;
*/
...
function sendmail( $to_from, $subject, $htmlMessage, $recieveFromCustomer = false){
    ...
    if( !($ret = $mailer->Send()) ) $sendmail_error = $mailer->ErrorInfo;
    return $ret;
}
...
class SMTPMailer extends PHPMailer {
	...
	{	
		...
		$this->Port = $settings['smtp']['port'];
		$this->SMTPDegug = $settings['smtp']['debug'];
		$this->Host = $settings['smtp']['host'];
	...
	public function Send(){
		...
		if($success && $this->logSuccesses){
		    ...
		    $to = is_array($this->to) && $this->to ? implode(",", $this->to[0]) : $this->to;
		    $row = "[ ".date('Y-m-d H:i:s')." ] ".$this->From." -> ".$to." [ ".$this->Subject." ] ".($this->logContent?$this->AltBody:'')."\r\n";
